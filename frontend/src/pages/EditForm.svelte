  ...
  const { form, errors } = createForm({
    ...
    onSubmit: async (values) => {
      try {
        ...
        await axios.patch(`http://localhost:8000/api/posts/${id}/update`, {title: values.title, body: values.body, user_id: values.user_id});
...
<form use:form>
  ...
  <dl>
    ...
    <dd><input type="text" name="title" id="title" placeholder="タイトル" value={post.title}></dd>
    {#if $errors.title}
      <p class="valid-msg">{$errors.title}</p>
    {/if}
    <dt><label for="body">本文</label></dt>
    <dd><textarea name="body" id="body" cols="30" rows="10" placeholder="本文" value={post.body}></textarea></dd>
    {#if $errors.body}
